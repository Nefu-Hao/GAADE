...
class simple_GC_DEC(nn.Module):
    ...
    def fit(self, X,adj,  lr=0.001, max_epochs=5000, update_interval=3, trajectory_interval=50,weight_decay=5e-4,opt="sgd",init="louvain",n_neighbors=10,res=0.4,n_clusters=10,init_spa=True,tol=1e-3):
        ...
        # if init=="hmm":
        #  print("Initializing cluster centers with Hidden Markov Model, n_clusters known")
        #  self.n_clusters = n_clusters
        #  hmm_model = HMM(self.n_clusters)  # Assuming you have implemented the HMM class for clustering
        #  if init_spa:
        #      #------HMM use exp and spatial
        #      X_np = features.detach().numpy()
        #  else:
        #      #------HMM only use exp info, no spatial
        #     X_np = X

        #     y_pred = hmm_model.fit_predict(X_np)  # Fit the HMM model and get the predicted cluster labels
